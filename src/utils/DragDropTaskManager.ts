...
export const updateTaskTagsForColumnMove = async (
	...
	if (sourceColumn.colType === "namedTag" && sourceColumn.coltag) {
		let sourceTag = sourceColumn.coltag.startsWith("#")
			? sourceColumn.coltag
			: `#${sourceColumn.coltag}`;
		...
		if (sourceTag.endsWith("/")) {
			...
			updatedTask.tags = updatedTask.tags.filter(
				(tag) => tag !== tagWithoutSlash && !tag.startsWith(sourceTag)
			);
		} else {
	...
	if (targetColumn.coltag) {
		let targetTag = targetColumn.coltag.startsWith("#")
			? targetColumn.coltag
			: `#${targetColumn.coltag}`;
			...
			Object.keys(tasksByFilePath).forEach((filePath) => {
				if (allTasks.Pending[filePath]) {
					...
					allTasks.Pending[filePath] = [
						...
						...allTasks.Pending[filePath].filter(
							(t) => !taskIdsByFilePath[filePath].includes(t.id)
						),
					];
				}
			});
