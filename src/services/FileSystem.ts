...
export const fs: typeof NodeFS = Platform.isDesktopApp
	...
	: null;
export const fsPromises: typeof NodeFS.promises = Platform.isDesktopApp
	? fs.promises
	: null!;
export const os: typeof NodeOS = Platform.isDesktopApp
	? window.require("node:os")
	: null;
export const path: typeof NodePath = Platform.isDesktopApp
	? window.require("node:path")
	: null;
export const url: typeof NodeUrl = Platform.isDesktopApp
	? window.require("node:url")
	: null;
export const zlib: typeof NodeZlib = Platform.isDesktopApp
	? window.require("node:zlib")
	: null;
...
export class NodePickedFile implements PickedFile {
	...
	async read(): Promise<ArrayBuffer | SharedArrayBuffer> {
...
export class NodePickedFolder implements PickedFolder {
	...
	async list(): Promise<(PickedFile | PickedFolder)[]> {
		let { filepath } = this;
		let files: NodeFS.Dirent[] = await fsPromises.readdir(filepath, {
			withFileTypes: true,
		});
		let results = [];
